...
void BlackBoxLearner::from_xml(TiXmlNode *node)
{
  ...
  rosban_utils::xml_tools::try_read<double>(node, "allowed_time", allowed_time);
  ...
  // Value approximator
  ValueApproximatorFactory().tryRead(node, "value_approximator", value_approximator);
  rosban_fa::OptimizerTrainerFactory().tryRead(node, "policy_trainer", policy_trainer);
}
