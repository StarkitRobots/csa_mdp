...
namespace csa_mdp
{
ExtraTreesApproximator::ExtraTreesApproximator() : nb_samples(100)
{
}
...
std::unique_ptr<starkit_fa::FunctionApproximator> ExtraTreesApproximator::train(
    const Policy& policy, const Problem& problem, Problem::ValueFunction current_value_function, double discount,
    std::default_random_engine* engine)
{
  ...
  Eigen::MatrixXd inputs = starkit_random::getUniformSamplesMatrix(problem.getStateLimits(), nb_samples, engine);
  Eigen::VectorXd observations(nb_samples);
  // Creating reward predictor task
  MultiCore::StochasticTask rp_task;
  rp_task = [this, &policy, &problem, discount, current_value_function, &inputs,
             &observations](int start_idx, int end_idx, std::default_random_engine* thread_engine) {
    for (int sample = start_idx; sample < end_idx; sample++)
    {
      Eigen::VectorXd state = inputs.col(sample);
      double mean, var;
      predictor->predict(state, policy, problem.getResultFunction(), current_value_function, discount, &mean, &var,
                         thread_engine);
...
void ExtraTreesApproximator::fromJson(const Json::Value& v, const std::string& dir_name)
{
  ...
  starkit_fa::TrainerFactory().tryRead(v, "trainer", dir_name, &trainer);
