...
double OpenLoopPlanner::sampleLookAheadReward(const Problem & p,
                                              const Eigen::VectorXd & initial_state,
                                              const Eigen::VectorXd & next_actions,
  ...
  rhoban_bbo::Optimizer::RewardFunc reward_function =
    ...
    (const Eigen::VectorXd & next_actions, std::default_random_engine * engine)
    {
      ...
      for (int rollout = 0; rollout < this->rollouts_per_sample; rollout++) {
        ...
        double rollout_reward = this->sampleLookAheadReward(p, state, next_actions,
                                                            &final_state, &trial_terminated, engine);
        // If rollout has not ended with a terminal status, use policy to end
        // the trial
        if (!trial_terminated) {
          double future_reward = p.sampleRolloutReward(final_state, policy,
                                                       trial_length - look_ahead,
                                                       this->discount, engine);
