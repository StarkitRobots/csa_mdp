...
public:
  ...
  virtual Eigen::VectorXd optimize(const Eigen::VectorXd& input, const Eigen::MatrixXd& action_limits,
                                   ...
                                   double discount, std::default_random_engine* engine) const override;

  typedef std::function<void(int start, int end, std::default_random_engine* engine)> AOTask;

  virtual AOTask getTask(const Eigen::VectorXd& input, const Eigen::MatrixXd& actions,
                         std::shared_ptr<const Policy> policy, Problem::ResultFunction result_function,
                         Problem::ValueFunction value_function, double discount, Eigen::VectorXd& results) const;
...
private:
  ...
  int nb_simulations;
  int nb_actions;
  std::unique_ptr<starkit_fa::Trainer> trainer;
};
