...
private:
  ...
  regression_forests::TrainingSet getTrainingSet(const std::vector<Sample>& samples,
                                                 std::function<bool(const Eigen::VectorXd&)> is_terminal,
                                                 const Config& conf,
                                                 std::shared_ptr<regression_forests::Forest> q_value, int start_idx,
                                                 int end_idx);

  /// Perform one step of update on the Q-value.
  std::unique_ptr<regression_forests::Forest> updateQValue(const std::vector<Sample>& samples,
...
public:
  ...
  std::unique_ptr<regression_forests::Forest> generateQValue(const std::vector<Sample>& samples,
                                                             std::function<bool(const Eigen::VectorXd&)> is_terminal,
                                                             Config& conf);
