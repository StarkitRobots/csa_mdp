...
public:
  ...
  std::unique_ptr<starkit_fa::FATree> trySplit(int split_dim, const std::vector<Eigen::VectorXd>& initial_states,
  ...
  /// Return the FATree built to replace current approximator and update score
  std::unique_ptr<starkit_fa::FATree> tryLinearSplit(int action_id, const std::vector<Eigen::VectorXd>& initial_states,
                                                    std::default_random_engine* engine, double* score);
