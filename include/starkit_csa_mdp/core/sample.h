...
public:
  ...
  Sample();
  Sample(const Eigen::VectorXd& state, const Eigen::VectorXd& action, const Eigen::VectorXd& next_state, double reward);

  /// Read a set of Samples from a csv file, user needs to specify the index of the columns
  static std::vector<Sample> readCSV(const std::string& path, const std::vector<size_t>& src_state_cols,
